        ...
        if (this.animating) { 
            ...
            for (let i = 0; i < image.width; i++)
            {
              for (let j = 0; j < image.height; j++)
              {
                ...
                {
                    s.pixels[s.width * j + i] = px[image.width * 
                        s.map(j, (image.height - h) / 2, (image.height + h) / 2 - 1, 0, image.height - 1) + 
                        s.map(i, (image.width - w) / 2, (image.width + w) / 2 - 1, 0, image.width - 1)];
                }
              }
            }
            ...
            */
            ...
            for (let i = 0; i < image.width; i++)
            {
              for (let j = 0; j < image.height; j++)
              {
                ...
                && j < (image.height + h) / 2))//check that pixels are within the new frame
                {
                    let mapY = s.map(j, (image.height - h) / 2, (image.height + h) / 2, 0, image.height);
                    let mapX = s.map(i, (image.width - w) / 2, (image.width + w) / 2, 0, image.width);

                    //console.log(j, (image.height - h) / 2, (image.height + h) / 2);
                   
                    //let index = 4 * (j * image.width + i);
                    let index = 4 * (s.width * j + i);
                    let index2 = (image.width * mapY + mapX);
                    //s.pixels[4 * j * image.width + i + offset] = px[mapX * image.width + mapY];
                    //(j * image.width + i)

                    s.pixels[index] = px[index];
                    s.pixels[index+1] = px[index + 1];
                    s.pixels[index+2] = px[index + 2];
                    s.pixels[index+3] = px[index + 3];
                }
              }
            }
